 ...
 ************************************************************************************/
 void initTableStorage() {
	 // Initialize those other variables
	 recordKey = -1;									// Default key to record is -1
	 tmpCounter = 0;									// Start recording the first pulse
	 ...
	 tmpKeyWidth[100] = 0;
	 recordInSession = false;							// No recording is going on yet
 }
...
void playNote(uint16_t noteFreq, uint8_t n, uint16_t dur, uint16_t durN) {
	...
	TCF1_CTRLA = 0x00;					// Turn off sound
}
...
void READ_KEY() {
	...
	do {
		...
		while(SCAN_KEYPAD() != -1){}			// Wait for key to be released
...
void storeKeyToTable() {
	...
	for(int i = 1; i != tmpCounter; ++i) {
		...
		keyWidth[recordKey][i] = tmpKeyWidth[i];
		tmpKeyWidth[i] = 0;
	}
}
...
ISR(TCF0_CCA_vect) {
	...
	if( (tmpCounter < 101) ) {
		tmpKeyWidth[tmpCounter] = TCC1_CNT;
		tmpCounter++;
