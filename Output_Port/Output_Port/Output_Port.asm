 ...
 * Authors:  Dr. Schwartz, Colin Watson, Rachel Johnson
 *
...
.org 0x200
MAIN:
	ldi R16, 0b10011		;Configure the PORTH bits 4, 1 and 0 as outputs. 
	sts PORTH_DIRSET, R16 	;  These are the CS0(L), RE(L), and RE(L) outputs. 
							;  (CS0 is bit 4; ALE1 RE is bit 1; WE is bit 0)
							; see 8385, Table 33-7
...
TEST:
	...
	; Set to read input port
	;ldi R16, 0b10001
	;sts PORTH_OUTSET, R16

	ld R17, X						; read the input port into R16
	st X, R17
